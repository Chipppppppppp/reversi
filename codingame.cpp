...
std::unordered_map<State, State::Position> book;
...
int alpha_beta(const State& state, int depth, int a, int b, State::Position& pos) {
    if (clock() - start_time >= time_limit) return 0;
    if (depth == 0) return state.eval();
    bool flag = true;
    for (int i = 0; i < 64; ++i) if (State::Position p{i}; state.is_putable(state.turn, p)) {
        flag = false;
...
std::pair<State::Position, int> alpha_beta_move() noexcept {
    start_time = clock();
    State::Position pos;
    int res;
...
void input_move() {
    ...
    for (int i = 0; i < 8; ++i) {
        std::cin >> s;
        for (int j = 0; j < 8; ++j) if (s[j] != '.' && !state.exists(State::black, {i, j}) && !state.exists(State::white, {i, j})) {
            state.move({i, j});
            state.pass();
            flag = true;
        }
